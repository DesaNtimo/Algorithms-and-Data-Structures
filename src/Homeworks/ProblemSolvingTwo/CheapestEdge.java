    ...
    public static void main(String[] args) throws IOException {
        ...
        for (int i = 1; i < n; i++) {
            ...
            for (int j = 1; j < 18; j++) {
                ...
                minEdge[i][j] = Math.min(minEdge[i][j - 1], minEdge[up[i][j - 1]][j - 1]);
            }
        }
        ...
        for (int i = 0; i < m; i++) {
            ...
            if (u != v) {
                for (int j = 17; j >= 0; j--) {
                    if (up[u][j] != up[v][j]) {
                        ans = Math.min(ans, Math.min(minEdge[u][j], minEdge[v][j]));
                        u = up[u][j];
                        v = up[v][j];
                    }
                }
